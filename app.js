...
var App = angular.module('emulationApp', []);
var gamesShow = false;
...
App.controller('SystemsDisplay', function($scope, $http) {
	$http.get('data/systems.json').then(function(res){
          $scope.systems = res.data;                
        });
});

App.controller('GamesDisplay', function($scope, $http) {
	$http.get('data/games.json').then(function(res){
          $scope.systems = res.data;        
        });
});